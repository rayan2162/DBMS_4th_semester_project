...
if (isset($_POST['submit'])) {
    ...
    // Fetch selected course ID
    //$options = isset($_POST['options']) ? $_POST['options'] : array();

    if (!empty($options)) {
      foreach($options as $index=>$rowoptions){
        foreach ($rowoptions as $poId) {
            // Insert data for each combination of course and program outcome
            $sql = "INSERT INTO courseprofile(objects, rationale, CO, courseOutcomes, courseId, PO) VALUES ('$objects', '$rationale', '$name', '$outcomes', $courseId, $poId)";
